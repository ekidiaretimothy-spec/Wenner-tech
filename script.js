...
const supabaseReady = Boolean(
  window.WENNER_AUTH_ENABLED &&
  window.WENNER_SUPABASE_URL &&
  window.WENNER_SUPABASE_ANON_KEY &&
  !window.WENNER_SUPABASE_URL.includes("PASTE_") &&
  !window.WENNER_SUPABASE_ANON_KEY.includes("PASTE_") &&
  window.supabase
);
const wennerAuth = supabaseReady
  ? window.supabase.createClient(window.WENNER_SUPABASE_URL, window.WENNER_SUPABASE_ANON_KEY)
  : null;
...
async function getBackendUser() {
  if (!wennerAuth) return null;
  const { data, error } = await wennerAuth.auth.getUser();
  if (error) return null;
  return data.user || null;
}

async function protectServicePages() {
  const protectedPage = document.body.dataset.requiresAuth === "true";
  if (!protectedPage) return;

  if (!wennerAuth) {
    const currentPage = getCurrentPageName();
    window.location.href = `signin.html?next=${encodeURIComponent(currentPage)}&setup=required`;
    return;
  }
  ...
  if (!user) {
    const currentPage = getCurrentPageName();
    window.location.href = `signin.html?next=${encodeURIComponent(currentPage)}`;
  }
}

async function updateAuthLinks() {
  const signedIn = Boolean(await getBackendUser());
...
function updatePrices(currencyCode) {
  ...
  priceRanges.forEach((price) => {
    ...
    const baseText = `Base: ₦${min.toLocaleString("en-NG")} - ₦${max.toLocaleString("en-NG")}`;
    price.textContent = `${formatCurrency(min, selected)} - ${formatCurrency(max, selected)}`;
...
if (signinForm) {
  ...
  if (!wennerAuth && authMessage) {
    authMessage.textContent = "Backend security is not active yet. Open supabase-config.js, paste your Supabase Project URL and anon public key, then set WENNER_AUTH_ENABLED to true.";
  }

  if (params.get("setup") === "required" && authMessage) {
    authMessage.textContent = "This page is protected. Supabase must be configured before visitors can sign in securely.";
  }

  getBackendUser().then((user) => {
    if (user && authMessage) {
      authMessage.textContent = "You are already signed in. You can now open the service pages.";
    }
  });
  ...
  signinForm.addEventListener("submit", async (event) => {
    ...
    if (!wennerAuth) {
      if (authMessage) {
        authMessage.textContent = "Secure backend is not configured yet. Please set up Supabase first in supabase-config.js.";
      }
      return;
    }

    const nextPage = params.get("next") || "contact.html";
    const redirectTo = `${window.location.origin}${window.location.pathname.replace("signin.html", nextPage)}`;
    const { error } = await wennerAuth.auth.signInWithOtp({
      email,
      options: {
    ...
    if (authMessage) {
      authMessage.textContent = error
        ...
        : "Verification email sent. Open your email and click the secure link to continue.";
    }
  });
}
